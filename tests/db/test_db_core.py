...
class TestDbCore:
    ...
    def test_db_add(
        ...
        assert len(session.exec(select(NotificationModel)).all()) == len(notifications)
    ...
    @classmethod
    def test_db_client_fetch_multi(cls, notifications: tuple[NotificationModel, ...], user0: UserModel) -> None:
        db_notifications = NotificationClient().fetch_all()
        assert len(db_notifications) == len(notifications)
        assert dict(db_notifications[1][0]) == dict(notifications[1])
        assert dict(db_notifications[1][1]) == dict(user0)

    @classmethod
    def test_fetch_notifications_by_user_id(
        cls, user0: UserModel, notifications: tuple[NotificationModel, ...]
    ) -> None:
        db_notifications = NotificationClient().fetch_by_user_id(user0.id)
        assert len(db_notifications) == len(
            [notification for notification in notifications if notification.user_id == user0.id]
        )
