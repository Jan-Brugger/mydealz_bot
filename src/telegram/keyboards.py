...
def start(
    ...
    for notification in sorted(notifications)[i_start:i_end]:
        ...
        keyboard.add(
            InlineKeyboardButton(
                query.strip(),
                callback_data=NotificationCB.new(Actions.VIEW, notification.id),
            ),
        )
    ...
    if page:
        pagination.append(
            InlineKeyboardButton('<<', callback_data=HomeCB.new(page=page - 1)),
        )
    ...
    keyboard.add(
        InlineKeyboardButton(
            '➕ Suchbegriff hinzufügen',  # noqa: RUF001
            callback_data=AddNotificationCB.new(),
        ),
    )
...
def notification_commands(notification: NotificationModel) -> InlineKeyboardMarkup:
    ...
    keyboard.row(
        InlineKeyboardButton(
            '✏️️ Suchbegriff ändern',
            callback_data=NotificationCB.new(Actions.UPDATE_QUERY, notification.id),
        ),
        InlineKeyboardButton(
            '❌ Löschen',
            callback_data=NotificationCB.new(Actions.DELETE, notification.id),
        ),
    )
    keyboard.row(
        InlineKeyboardButton(
            '💸 Minimalpreis ändern',
            callback_data=NotificationCB.new(Actions.UPDATE_MIN_PRICE, notification.id),
        ),
        InlineKeyboardButton(
            '💰 Maximalpreis ändern',
            callback_data=NotificationCB.new(Actions.UPDATE_MAX_PRICE, notification.id),
        ),
    )
    ...
    keyboard.row(
        InlineKeyboardButton(
            hot_toggle_text,
            callback_data=NotificationCB.new(Actions.TOGGLE_ONLY_HOT, notification.id),
        ),
        InlineKeyboardButton(
            '➕ Suchbegriff hinzufügen',  # noqa: RUF001
            callback_data=AddNotificationCB.new(),
    ...
    keyboard.row(
        InlineKeyboardButton(
            ...
            callback_data=NotificationCB.new(
                Actions.TOGGLE_SEARCH_DESCR,
                notification.id,
            ),
        ),
...
def deal_kb(deal_link: str, notification: NotificationModel) -> InlineKeyboardMarkup:
    ...
    keyboard.row(
        InlineKeyboardButton(
            '🗒️ Übersicht',
            callback_data=NotificationCB.new(Actions.VIEW, notification.id, reply=True),
        ),
        InlineKeyboardButton(
            '❌ Löschen',
            callback_data=NotificationCB.new(Actions.DELETE, notification.id, reply=True),
        ),
...
def settings(user: UserModel) -> InlineKeyboardMarkup:
    ...
    toggles = [
        (
            SettingsCB.new(action=SettingsActions.TOGGLE_MYDEALZ),
            user.search_mydealz,
            'mydealz.de',
        ),
        (
            SettingsCB.new(action=SettingsActions.TOGGLE_PREISJAEGER),
            user.search_preisjaeger,
            'preisjaeger.at',
        ),
        (
            SettingsCB.new(action=SettingsActions.TOGGLE_IMAGES),
            user.send_images,
            'Deal-Bilder',
        ),
    ]
    for toggle in toggles:
        keyboard.add(
            InlineKeyboardButton(
                f'✅ {toggle[2]}' if toggle[1] else f'❌ {toggle[2]}',
                callback_data=toggle[0],
            ),
        )
...
def broadcast_message(message_id: int) -> InlineKeyboardMarkup:
    ...
    keyboard.row(
        InlineKeyboardButton(
            '✅ Ja',
            callback_data=BroadcastCB.new(message_id=message_id),
        ),
        InlineKeyboardButton('❌ Nein', callback_data=HomeCB.new()),
    )
